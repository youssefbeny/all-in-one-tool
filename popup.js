console.log("Gaming Tools Suite - Popup script chargé");
...
// ================================
// FONCTIONNALITÉS TIMER SPEEDRUN
// ================================

function loadCurrentHotkey() {
    chrome.runtime.sendMessage({ action: "getHotkey" }, (response) => {
        if (chrome.runtime.lastError) {
            console.log("Erreur lors du chargement de la hotkey:", chrome.runtime.lastError);
            return;
        }
...
function setupTimerEventListeners() {
    ...
    if (toggleTimerBtn) {
        toggleTimerBtn.addEventListener('click', function() {
            console.log("Toggle timer cliqué");
                ...
                if (tabs[0]) {
                    chrome.tabs.sendMessage(tabs[0].id, { action: "toggleTimer" }, (response) => {
                        if (chrome.runtime.lastError) {
                            ...
                            showStatus("Timer activé/désactivé !", true);
                        }
                    });
                }
            });
    ...
    hotkeyButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            console.log("Hotkey button cliqué:", btn.dataset.key);
...
function setHotkey(keyCode) {
    chrome.runtime.sendMessage({ 
        ...
        if (chrome.runtime.lastError) {
            console.log("Erreur sauvegarde hotkey:", chrome.runtime.lastError);
            return;
        }
        if (response && response.success) {
            currentActiveKey = keyCode;
            updateHotkeyDisplay();
            showStatus(`Touche changée : ${getKeyDisplayName(keyCode)}`, true);
...
// ================================
// FONCTIONNALITÉS SUBWAY SURFERS
// ================================
...
function setupSubwayEventListeners() {
    ...
    if (backupBtn) {
        backupBtn.addEventListener('click', function() {
            console.log("Backup button cliqué");
            doBackup();
        });
    }
    ...
    if (restoreBtn) {
        restoreBtn.addEventListener('click', function() {
            console.log("Restore button cliqué");
            doRestore();
        });
    }
    ...
    if (save100Btn) {
        save100Btn.addEventListener('click', function() {
            console.log("Save 100% button cliqué");
            doSave100();
        });
    }
    ...
    if (cleanDataBtn) {
        cleanDataBtn.addEventListener('click', function() {
            console.log("Clean data button cliqué");
            doCleanData();
        });
    }
}
...
function doBackup() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (!tabs[0]) {
            showStatus("Aucun onglet actif trouvé", false);
            return;
        }
        ...
        chrome.scripting.executeScript({
            ...
            if (chrome.runtime.lastError) {
                showStatus("Erreur: Rechargez la page Subway Surfers", false);
                console.error("Backup error:", chrome.runtime.lastError);
            } else {
                showStatus("Backup créé avec succès !", true);
            }
        });
    });
}
            ...
            chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                ...
                chrome.scripting.executeScript({
                    ...
                    if (chrome.runtime.lastError) {
                        ...
                        showStatus("Sauvegarde restaurée ! Rechargez la page.", true);
                    }
                });
            });
        };
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        chrome.scripting.executeScript({
            ...
            if (chrome.runtime.lastError) {
                ...
                showStatus("Save 100% appliqué ! Rechargez la page.", true);
            }
        });
    });
}
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        chrome.scripting.executeScript({
            ...
            if (chrome.runtime.lastError) {
                ...
                showStatus("Données supprimées. Rechargez la page.", true);
            }
        });
    });
}
...
function setupZqsdEventListeners() {
    ...
    if (activateBtn) {
        activateBtn.addEventListener('click', function() {
            console.log("Activate ZQSD button clicked");
            activateZqsd();
        });
    }
    ...
    if (deactivateBtn) {
        deactivateBtn.addEventListener('click', function() {
            console.log("Deactivate ZQSD button clicked");
            deactivateZqsd();
        });
    }
}
...
function activateZqsd() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        chrome.scripting.executeScript({
            ...
            if (chrome.runtime.lastError) {
                showStatus("Erreur: Impossible d'activer ZQSD", false);
                console.error("ZQSD activation error:", chrome.runtime.lastError);
            } else {
                showStatus("ZQSD activé avec succès !", true);
            }
        });
    });
}
...
function deactivateZqsd() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        chrome.scripting.executeScript({
            ...
            if (chrome.runtime.lastError) {
                showStatus("Erreur: Impossible de désactiver ZQSD", false);
                console.error("ZQSD deactivation error:", chrome.runtime.lastError);
            } else {
                showStatus("ZQSD désactivé !", true);
            }
        });
    });
}
...
function executeZqsdScript(active) {
    const SCRIPT_KEY = "myScriptHandler"; 
    ...
    if (active) { 
        ...
        const handler = (e) => { 
            ...
            if (e.key === '1') { 
                ...
                showPopup('<span class="material-icons" style="vertical-align:middle; margin-right:5px;">play_arrow</span> Script used (key 1)', "#4caf50"); 
                return; 
            } 
        ...
        showPopup('<span class="material-icons" style="vertical-align:middle; margin-right:5px;">check_circle</span> Script activated (ZQSD)', "#4caf50"); 
    } else { 
        if (!window[SCRIPT_KEY]) return; 
        ...
        showPopup('<span class="material-icons" style="vertical-align:middle; margin-right:5px;">cancel</span> Script deactivated (ZQSD)', "#f44336"); 
    }
}

// ================================
// FONCTIONS SUBWAY SURFERS
// ================================
        ...
        request.onsuccess = function (event) {
            ...
            getRequest.onsuccess = function () {
                ...
                if (data) {
                    ...
                    console.warn("Aucune donnée trouvée pour la sauvegarde.");
                    resolve(false);
                }
            };
...
function restoreData(fileContent) {
    ...
    request.onsuccess = function (event) {
        ...
        try {
            ...
            console.error("Erreur de format du fichier:", error);
        }
    };
...
function applySave100() {
    ...
    request.onsuccess = function (event) {
        ...
        putRequest.onsuccess = function () {
            console.log("Save 100% appliqué avec succès.");
            location.reload();
        };
        putRequest.onerror = function () {
            console.error("Erreur lors de l'application du save 100%.");
        };
    };
